...
- S04 is **IN PROGRESS**: focused review round 2 (`spikes/S04-host-canary-review.md`) blocked on
  ...
  id) but cannot override the higher-priority CLI profile. The confounded pre-remediation Codex
  injection result awaits the isolated rerun, which the vendor usage limit is blocking. The native
...
After the Codex usage limit resets (14:50 local, 2026-09-19), run the six isolated Codex canaries
(`baseline`/`project-control`/`project-injection` × 0.155.0/0.154.0) and record them. Then run focused
...
- S04 local: `/tmp/pcc-s04-20260919/bin/python run_s04.py` → 54 tests OK, 5 skips because the host
  ...
  rerun passed; the Codex isolated rerun is pending.
