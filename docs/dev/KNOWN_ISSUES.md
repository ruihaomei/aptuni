...
| KI-020 | High | S05A proved the common identity contract on synthetic fixtures only. Real MarginNote 4 export identity (vendor attribute stability, branch export shape) and live GitHub pagination/redirect/rate-limit behavior are unverified. | No vendor node ID is trusted by default. The MarginNote S05B suite needs a maintainer-created export series (no-op, edit, move, duplicate, delete/recreate, branch, app restart) before the provider ships; GitHub S05B covers live API behavior. |
