...
| KI-020 | High | MarginNote OPML identity. S05B replayed the production reconciler over real MarginNote 4 backup history (298 transitions, 131,902 nodes): no cross-content link; R1/R3 adopted (ADR-0006 amendment), review items −61% but still ≈34/sync in the harness. The real exporter's OPML shape (text choice, branch export) is unverified, and the locator must be minimized before ingest. GitHub rename remains fixture-proven only. | Keep `source.marginnote` at `preview`. Close after a real full + branch OPML export replays cleanly, locator@2 is reviewed, and review burden is acceptable. |
