...
| KI-020 | High | S05A proved MarginNote identity only on synthetic fixtures. Real MarginNote 4 export identity (vendor attribute stability and branch export shape) remains unverified. GitHub S05B is now live-verified and shipped locally at `0190cf6`; the authorized sample contains deletion/history but no rename commit. | No MarginNote vendor node ID is trusted by default. Inspect the authorized local MN4 database/backups and exports; require a maintainer-created sanitized no-op/edit/move/duplicate/delete-recreate/branch/restart series if history is insufficient. GitHub rename remains fixture-proven and is not claimed as live-observed. |
