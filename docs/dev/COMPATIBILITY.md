...
**Status:** Draft; S03/S04/S12 and the first scaffold fill exact versions before acceptance.
...
at `/opt/homebrew/bin/python3.13` with its SQLite 3.53.2. Claude Code 2.1.87 (`~/.local/bin/claude`) and Codex CLI 0.153.4 (ChatGPT desktop bundle, not on
PATH; research 06) are the S04
planning snapshot. Gate 0 accepts the Vault write protocol only on this baseline; unknown, network,
