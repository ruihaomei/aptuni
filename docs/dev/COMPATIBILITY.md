...
| Codex 0.155.0 + 0.154.0 | Config/profiles | no profile/`-c`/trusted-project override weakens the above; `~/.codex` and bundled adapter path outside writable roots | Isolated A/B (temporary `CODEX_HOME` with one trust entry, identical in every mode): a trusted control with no `.codex/` blocked write/TCP/AF_UNIX, while the project `.codex/config.toml` alone selected `danger-full-access` and made all three succeed. Outer observers confirmed each effect in both versions. Core project-file scanning yields `unbundled_entry_visible`, and the successful effects add write/socket reasons. |
