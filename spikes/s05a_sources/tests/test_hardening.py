...
from s05a.contract import CandidateDelta, ContractError, Extension, Operation, SourceLocator, canonical_json
...
class LedgerOrderingTests(unittest.TestCase):  # F1, F4, F6
    def setUp(self) -> None:
        self.fx = TempFolder()
        self.addCleanup(self.fx.cleanup)

    def test_content_flip_flop_is_applied_not_swallowed(self) -> None:
        self.fx.write("a.md", "v1")
        s1 = self.fx.scan()
        ledger = Ledger()
        ledger.apply(s1.delta)
        subject = s1.snapshot.items[0].locator.subject_id
        ledger.add_fact("f", "x", evidence=(("src-folder", subject),))
        ...
        self.assertEqual("applied", ledger.apply(s4.delta))
        self.assertEqual("needs_reevaluation", ledger.facts["f"].status)
        self.assertEqual("duplicate", ledger.apply(s4.delta))
        self.fx.write("a.md", "v3")
        self.assertEqual("applied", ledger.apply(self.fx.scan(s4).delta))

    def test_mutated_delta_fails_integrity_check(self) -> None:
        self.fx.write("a.md", "v1")
        delta = self.fx.scan().delta
        fields = delta.operations[0].after.extension.fields  # type: ignore[union-attr]
        fields["relative_path"] = "../../etc/passwd"  # type: ignore[index]
        with self.assertRaises(LedgerError):
            Ledger().apply(delta)

    def test_intake_gates_unknown_versions_to_review_and_rejects_invalid(self) -> None:
        future = SourceLocator("src-folder", "folder", "d1", Extension("folder.locator", 9, {"relative_path": "a"}))
        ledger = Ledger()
        ledger.apply(CandidateDelta.build("src-folder", None, "s1", ("p", "1"), (Operation.add(future, "h"),)))
